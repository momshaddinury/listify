...
class TasksViewModel extends GetxController {
  ...
  Future createNewTask(String title, description, dateTime, priority) => _tasksRepository.createNewTask(title, description, dateTime, priority);

  Future updateTask(uid, title, description, dateTime, priority) => _tasksRepository.updateTask(uid, title, description, dateTime, priority);

  Future completeTask(uid) => _tasksRepository.completeTask(uid);

  Future undoCompleteTask(uid) => _tasksRepository.undoCompleteTask(uid);

  Future removeTodo(uid) => _tasksRepository.removeTodo(uid);
